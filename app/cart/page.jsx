...
function Cart() {
	...
	const deleteCartItemFromList = async (id) => {
		console.log("Deleting item with ID:", id); // Confirm the ID being passed
		
		try {
			const res = await CartApis.deleteCartItem(id);
			console.log("Full Delete Response:", res); // Log the full response object
	
			// If the status is 200 (OK) or 204 (No Content), proceed to remove the item
			if (res?.status === 200 || res?.status === 204) {
				setCart((oldCart) => oldCart.filter(item => item.documentId !== id));
				console.log(`Item with ID ${id} removed from the cart`);
			} else {
				console.error(`Failed to delete item. Status code: ${res?.status}`);
			}
		} catch (error) {
						...
						<ul className="space-y-4">
							{cart?.map((item) => (
								<li className="flex items-center gap-4">
									<img
										src={item?.product?.banner?.formats?.thumbnail?.url}
										alt=""
									...
									<div className="flex items-center justify-end flex-1 gap-2">
										...
										<button onClick={() => deleteCartItemFromList(item?.documentId)} className="text-gray-600 transition hover:text-red-600">
